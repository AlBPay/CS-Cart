...
class Fondy
{
    ...
    {
        $url = ($capture) ? 'https://payment.albpay.io/api/capture/order_id' : 'https://payment.albpay.io/api/checkout/url/';
        $ch = curl_init();
    ...
    function isPaymentValid($settings, $response)
    {
        ...
        if ($settings['merchant'] != $response['merchant_id']) {
            return 'Alliance bank_error_merchant';
        }
        ...
        if ($signature != $responseSignature) {
            return 'Alliance bank_error_signature';
        }
        return true;
    }
}